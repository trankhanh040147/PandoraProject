...
public class ListStoreController extends HttpServlet {
	...
	protected void LoadStore(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		List<Store> storeList = storeServiceImpl.GetStoreByownerId((index - 1) * ItemsPerPage, ItemsPerPage,
				users.getId());
		...
		List<Store> storeListAll = storeServiceImpl.GetStoreByownerId(0, 200, users.getId());
		int totalItems = storeListAll.size();
		...
		// Dữ liệu xử lí
		req.setAttribute("listStore", storeList);
